...
export function createOrderRow(order) {
  ...
  const elapsedTime = order.elapsedTime || "";
  const saleId = String(order.saleId || order.id || "").replace(/^#/, "").trim();

  row.className = "order-row-link";
  row.tabIndex = 0;
  row.setAttribute("role", "link");
  row.setAttribute("aria-label", `Abrir edicao da venda de ${order.customer}`);
  if (saleId) {
    row.dataset.saleId = saleId;
  }

  row.innerHTML = `
    <td class="order-status-cell"><span class="${badgeClass} order-status-pill">${order.status}</span></td>
    <td data-label="Nome"><strong class="order-customer">${order.customer}</strong></td>
    <td data-label="Tempo"><span class="order-time">${elapsedTime}</span></td>
  `;
...
export function createProductCard(product) {
  ...
  item.append(productName, productPrice, meta);

  return item;
}

export function createClientCard(client) {
  const balanceDue = Number(client.balanceDue || client.saldo_devendo || 0);
  const item = document.createElement("article");
  ...
  balance.className = `client-card__balance ${balanceDue > 0 ? "client-card__balance--danger" : ""}`.trim();
  balance.textContent = `Saldo devendo: ${formatCurrency(balanceDue)}`;

  item.append(header, name, balance);
  return item;
}
...
function formatSaleDate(saleDate) {
  const normalizedDate = typeof saleDate === "string" && /^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/.test(saleDate)
    ? saleDate.replace(" ", "T")
    : saleDate;
